...
export async function GET(req: NextRequest) {
    ...
    // Check cache
    const cacheKey = makeCacheKey("finances", url.searchParams);
    ...
    try {
        const result = await query(
            `SELECT * FROM financial_events ORDER BY posted_date DESC LIMIT $1 OFFSET $2`,
            [perPage, offset]
        );

        const countResult = await query<{ total: string }>(`SELECT COUNT(*)::text as total FROM financial_events`);
        ...
        console.error("Finances API Error:", message);
